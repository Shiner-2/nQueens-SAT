...
def at_most_one(clauses, variables, new_variables):
    ...
    r = []
    c = []
    for i in range(lst+1,lst+p+1):
        r.append(i)
    for i in range(lst+p+1,lst+p+q+1):
        c.append(i)
    new_variables += r + c
    
    at_most_one_biominal_encoding(clauses, r)
    at_most_one_biominal_encoding(clauses, c)
    
    x = 0
    y = 0
    
    for i in range(len(variables)):
        clauses.append([r[x], -variables[i]])
        clauses.append([c[y], -variables[i]])
        x += 1
        y += 1
        if x == p:
            x = 0
